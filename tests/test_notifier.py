...
class TestWebhook:
    def test_webhook_called_when_configured(self, tmp_path, monkeypatch):
        ...
        n = Notifier(
            min_severity="High", alert_log_file=str(tmp_path / "a.log"), webhook_url="http://localhost:9999/hook"
        )
    ...
    def test_webhook_failure_is_swallowed(self, tmp_path, monkeypatch):
        ...
        n = Notifier(min_severity="High", alert_log_file=str(tmp_path / "a.log"), webhook_url="http://unreachable/hook")
        # Must not raise:
